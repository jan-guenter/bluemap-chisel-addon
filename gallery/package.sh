...
archive_temp="$(mktemp -d /tmp/bluemap-chisel-gallery.XXXXXX)"
...
(
  ...
  LC_ALL=C find . -type f -printf '%P\n' | LC_ALL=C sort |
    zip -q -X -9 "$archive_temp/chisel-gallery.zip" -@
)
unzip -tq "$archive_temp/chisel-gallery.zip"
cp "$archive_temp/chisel-gallery.zip" "$output_path"
