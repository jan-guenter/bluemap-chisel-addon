 ...
 *
 * Geometry and UV conventions are independently implemented against the
...
final class AthenaQuadEmitter {
    ...
    boolean emit(
        ...
        if (skipForTopOnly(renderSettings.isRenderTopOnly(), face)) {
            return true;
        }
    ...
    static boolean skipForTopOnly(boolean renderTopOnly, CubeFace face) {
        return renderTopOnly && face != CubeFace.UP;
    }
    ...
    static boolean shouldCull(
            boolean neighborCulling,
            boolean neighborCullingIdentical,
            boolean sameState
    ) {
        return neighborCulling || neighborCullingIdentical && sameState;
    }
